...
window.showSpaceInfo=function(idx){
  ...
  const bLevel=land?.buildings||0;
  const grp=s[5]; const gc=grp?GROUP_COLORS[grp]:'';
  const grpLabel=grp?`<span style="display:inline-block;background:${gc};color:#fff;border-radius:8px;padding:2px 9px;font-size:12px;font-weight:700;margin-bottom:4px;text-shadow:0 1px 2px #0003">같은 그룹 소유 필요</span>`:'';
  const ownerInfo=o?`<div class="info-owner" style="border-color:${o.color}"><span>${o.avatar||'★'}</span> <b>${esc(o.name)}</b> 팀 소유 ${bLevel>0?`· ${BUILD_ICONS[bLevel]} ${BUILDINGS[bLevel]}`:''}</div>`:'';
