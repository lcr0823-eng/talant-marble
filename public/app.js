...
async function doAction(type,more={}){if(busy)return;busy=true;try{if(type==='roll'){rolling=true;render();await new Promise(resolve=>setTimeout(resolve,650));}room=await api('/api/action',{roomId,playerId,type,...more});}catch(e){alert(e.message)}finally{rolling=false;busy=false;render()}}
